...
function openBioModal(slug) {
  ...
  if (pubs.length > 0) {
    ...
    const headerTitle = isPI ? 'Publications' : 'Conscious Brain Lab publications';

    const listItems = pubs.map((p, idx) => {
      const formattedCitation = (p.citation_html || p.citation || '')
        .replace(/<\/?strong>/gi, '')
        .replace(/&Amp;/g, '&amp;');
  ...
  modal.innerHTML = `
    <div class="modal-box">
      ...
      ${pubsHtml}
      <div style="display: flex; justify-content: space-between; align-items:center; border-top: 1px solid var(--border-color); padding-top: 1rem; margin-top: 1.25rem;">
